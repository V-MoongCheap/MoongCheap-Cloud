...
#   1. AWS 인증 확인
#   2. Managed Node Group desired → 0  (FE. Karpenter 컨트롤러도 여기서 함께 내려간다)
...
info "[1/4] Managed Node Group 축소 → 0"
for entry in "${MANAGED_NODEGROUPS[@]}"; do
  ng="${entry%%=*}"
  scale_nodegroup "${ng}" 0
done
...
for entry in "${MANAGED_NODEGROUPS[@]}"; do
  ...
  info "  ${ng}: ${st} desired=${desired} running=$(nodegroup_running_count "${ng}")"
done
